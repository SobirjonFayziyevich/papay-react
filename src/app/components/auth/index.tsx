...
export default function AuthenticationModal(props: any) {
  ...
  const handleUsername = (e: any) => {
    set_mb_nick(e.target.value); // evetni ichiga kirib valueni qabul qilib olamiz.
    // console.log(mb_nick);
  };
  const handlePhone = (e: any) => {
    set_mb_phone(e.target.value); // evetni ichiga kirib valueni qabul qilib olamiz.
    // console.log(mb_phone);
  };
  const handlePassword = (e: any) => {
    set_mb_password(e.target.value); // evetni ichiga kirib valueni qabul qilib olamiz.
  ...
  const handleSingupRequest = async () => {
    try {
      const is_fulfilled = mb_nick != "" && mb_password != "" && mb_phone != 0; //mb_phone 0 ga teng bulmasligi kerak.
  ...
  const handleLoginRequest = async () => {
    try {
      ...
      console.log(err);
      props.handleLoginClose(); // xatolik yuz berganida yopib yuborsin.
      sweetErrorHandling(err).then();
    }
  };
  // login bulganimdan sung ENTER tugmasini bosib kirish logikasi..
  const passwordKeyPressHandler = (e: any) => {
    try {
      if (e.key == "Enter" && props.signUpOpen) {// agar ket teng bulsa, ENTERga va qachonkiy props.signUpOpen bulganda.
